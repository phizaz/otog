		...
		<tr>
			...
			for ($i = 0; $i <= $cnt && $rank <= $maxRank; $i++) {
				...
				foreach($info[$i]['task'] as $task){
					echo '
					<div style="float: left; width: 192px;" align="center"><div style="padding: 5px;">
						<div style="font-weight: bold;">'.$task['task_id'].'. <a href="doc/'.$task['task_name_short'].'.pdf" target="_blank">'.$task['task_name'].'</a></div>
						<div>'.$task['text'].'</div>
						<div>คะแนน '.($task['score'] / 100.0 * $task['task_score']).' / '.$task['task_score'].'</div>
						<div>เวลา ';printf("%.2lf", $task['timeused']);echo ' ส่ง '.$task['submit_count'];
					echo '</div>
						</div>
					</div>';
				}
